...
{	
	...
	Login()
	{
		f1=new JFrame("Login");
		try {
			f1.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("\\Vehicle-Management-System-master\\src\\images\\loginbk.png")))));
		} catch (IOException e) {
			e.printStackTrace();
		}
		f1.setSize(300,400);
		...
		try {
			log=new JButton("Login",new ImageIcon(ImageIO.read(new File("\\Vehicle-Management-System-master\\src\\images\\loginico.png"))));
		} catch (IOException e) {
			e.printStackTrace();
		}
