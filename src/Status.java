...
public class Status implements ActionListener
{
	...
	Status()
	{
		...
		try {
			f8.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File(
					"\\Vehicle-Management-System-master\\src\\images\\adminback-1.png")))));
		} catch (IOException e) {
			e.printStackTrace();
		}
		f8.setSize(400,400);
		...
		try {
			search=new JButton("Check",new ImageIcon(ImageIO.read(new File(
					"\\Vehicle-Management-System-master\\src\\images\\schd.png"))));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		try {
			cancel=new JButton("Cancel",new ImageIcon(ImageIO.read(new File(
					"\\Vehicle-Management-System-master\\src\\images\\cancelico.png"))));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void actionPerformed(ActionEvent e) 
	{
		if(e.getSource()==search)
		{
			...
			try {
				...
				Connection cn=DriverManager.getConnection("jdc:oracle:thin:@Localhost:1521:xe","system","root");
				...
				if(b)
				{
					...
					try {
						f08.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File(
								"\\Vehicle-Management-System-master\\src\\images\\adminback-2.png")))));
					...
					try {
						ok=new JButton("Ok",new ImageIcon(ImageIO.read(new File(
								
								"\\Vehicle-Management-System-master\\src\\images\\ok.png"))));
					} catch (IOException ex) {
						ex.printStackTrace();
					}
					...
					try {
						edit=new JButton("Edit",new ImageIcon(ImageIO.read(new File(
								"\\Vehicle-Management-System-master\\src\\images\\clear.png"))));
					} catch (IOException ex) {
						ex.printStackTrace();
					}
		...
		if(e.getSource()==edit)
		{
			...
			try {
				f008.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File(
						"\\Vehicle-Management-System-master\\src\\images\\adminback-2.png")))));
			} catch (IOException ex) {
				ex.printStackTrace();
			}
			...
			try {
				done=new JButton("Done",new ImageIcon(ImageIO.read(new File(
						"\\Vehicle-Management-System-master\\src\\images\\save.png"))));
			} catch (IOException ex) {
				ex.printStackTrace();
			}
